...
// C++17 clamp
template <class _Tp>
inline const _Tp& (clamp)(const _Tp& __v, const _Tp& __a, const _Tp& __b) {
  return __v < __a ? __a : __b < __v ? __b : __v;
}

template <class _Tp, class _Compare>
inline const _Tp& (clamp)(const _Tp& __v, const _Tp& __a, const _Tp& __b, _Compare __comp) {
  return __comp(__v, __a) ? __a : __comp(__b, __v) ? __b : __v;
}
